...
public:
	...
	void add_immediate_to_eax(std::vector<uint8_t>* memoryBlock, uint32_t* dword){ init(memoryBlock, 6); addByte(0x03); addByte(0x05); addDword(dword); }
	void add_immediate_to_ebx(std::vector<uint8_t>* memoryBlock, uint32_t* dword){ init(memoryBlock, 6); addByte(0x03); addByte(0x1D); addDword(dword); }
